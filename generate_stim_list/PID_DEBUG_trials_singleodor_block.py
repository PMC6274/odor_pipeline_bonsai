...
def make_trial_list(odor_sequence: str, block_num: int):
    ...
    - Each block contains all odors exactly once (no replacement)
    - Order is randomized within each block
    - Total trials = len(odors) * block_num

    Example (block_num=2, odors=A B C):
    Block 1: B A C
    Block 2: C B A
    """
    trials = []
    ...
    for block_idx in range(1, block_num + 1):
        shuffled_odors = odors.copy()
        random.shuffle(shuffled_odors)

        for odor_name in shuffled_odors:
...
def save_bonsai_and_csv(
    ...
    block_num=10,
    txt_file="DEBUG_bonsai_6X10_flush.txt",
    csv_file="DEBUG_bonsai_6X10_flush_table.csv"
):
