...
For `3device_parallel_64mix.bonsai`, configure vacuum control in the existing
Hardware and Protocol sections:
...
The 64-mix launcher requires both values and maps them to `Define Device.vaccum port`
and `Initialize Device.Vacuum (ml/min)`. The rate must be finite and non-negative,
in ml/min (it is not limited to the olfactometer's 0–1000 range). Press **V** in
Bonsai to send the configured rate. Workflows without vacuum control do not receive
these properties. Validate this workflow without opening hardware with:

```powershell
.\scripts\run_experiment.ps1 experiments\your-session.yaml -Workflow 3device_parallel_64mix.bonsai -ValidateOnly
```
